...
var _ = Describe("readProgress", Label("unit", "transcoding"), func() {
	...
	It("drains the reader past a line the scanner cannot hold", func() {
		// bufio.Scanner tops out at 64 KiB a line and stops there. Whatever
		// follows still has to be read, or ffmpeg blocks writing into a full
		// pipe and the encode never ends.
		r := strings.NewReader(
			strings.Repeat("x", 128*1024) + "\nprogress=end\n",
		)
		readProgress(context.Background(), r, time.Second, func(Snapshot) {})
