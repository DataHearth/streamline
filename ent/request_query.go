...
func (_q *RequestQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Request, error) {
	...
	if _q.withRequester != nil || _q.withApprovedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withRequester; query != nil {
		if err := _q.loadRequester(ctx, query, nodes, nil,
			func(n *Request, e *User) { n.Edges.Requester = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withApprovedBy; query != nil {
		if err := _q.loadApprovedBy(ctx, query, nodes, nil,
			func(n *Request, e *User) { n.Edges.ApprovedBy = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RequestQuery) loadRequester(ctx context.Context, query *UserQuery, nodes []*Request, init func(*Request), assign func(*Request, *User)) error {
	ids := make([]uint32, 0, len(nodes))
	nodeids := make(map[uint32][]*Request)
	for i := range nodes {
...
func (_q *RequestQuery) loadApprovedBy(ctx context.Context, query *UserQuery, nodes []*Request, init func(*Request), assign func(*Request, *User)) error {
	ids := make([]uint32, 0, len(nodes))
	nodeids := make(map[uint32][]*Request)
	for i := range nodes {
		if nodes[i].request_approved_by == nil {
			continue
		}
		fk := *nodes[i].request_approved_by
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "request_approved_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RequestQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(request.Table, request.Columns, sqlgraph.NewFieldSpec(request.FieldID, field.TypeUint32))
	_spec.From = _q.sql
