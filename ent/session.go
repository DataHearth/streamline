...
func (*Session) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case session.FieldJti, session.FieldIP, session.FieldUserAgent:
			values[i] = new(sql.NullString)
		case session.FieldCreateTime, session.FieldUpdateTime, session.FieldExpiresAt, session.FieldRevokedAt, session.FieldLastSeenAt:
	...
	for i := range columns {
		...
		case session.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.user_sessions = new(uint32)
				*_m.user_sessions = uint32(value.Int64)
			}
		default:
