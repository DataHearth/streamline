...
func (_q *OIDCIdentityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OIDCIdentity, error) {
	...
	if query := _q.withOwner; query != nil {
		if err := _q.loadOwner(ctx, query, nodes, nil,
			func(n *OIDCIdentity, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OIDCIdentityQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*OIDCIdentity, init func(*OIDCIdentity), assign func(*OIDCIdentity, *User)) error {
	ids := make([]uint32, 0, len(nodes))
	nodeids := make(map[uint32][]*OIDCIdentity)
	for i := range nodes {
		if nodes[i].user_oidc_identities == nil {
			continue
		}
		fk := *nodes[i].user_oidc_identities
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_oidc_identities" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *OIDCIdentityQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(oidcidentity.Table, oidcidentity.Columns, sqlgraph.NewFieldSpec(oidcidentity.FieldID, field.TypeUint32))
	_spec.From = _q.sql
