...
func (*OIDCIdentity) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case oidcidentity.FieldProvider, oidcidentity.FieldSubject, oidcidentity.FieldEmail:
			values[i] = new(sql.NullString)
		case oidcidentity.FieldCreateTime, oidcidentity.FieldUpdateTime:
	...
	for i := range columns {
		...
		case oidcidentity.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				_m.user_oidc_identities = new(uint32)
				*_m.user_oidc_identities = uint32(value.Int64)
			}
		default:
...
func (_m *OIDCIdentity) QueryOwner() *UserQuery {
	return NewOIDCIdentityClient(_m.config).QueryOwner(_m)
}
