...
func (_q *EpisodeQuery) loadDownloadRecords(ctx context.Context, query *DownloadRecordQuery, nodes []*Episode, init func(*Episode), assign func(*Episode, *DownloadRecord)) error {
	...
	for _, n := range neighbors {
		fk := n.episode_download_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "episode_download_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "episode_download_records" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *EpisodeQuery) loadMediaFiles(ctx context.Context, query *MediaFileQuery, nodes []*Episode, init func(*Episode), assign func(*Episode, *MediaFile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint32]*Episode)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.MediaFile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(episode.MediaFilesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.episode_media_files
		if fk == nil {
			return fmt.Errorf(`foreign-key "episode_media_files" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *EpisodeQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(episode.Table, episode.Columns, sqlgraph.NewFieldSpec(episode.FieldID, field.TypeUint32))
	_spec.From = _q.sql
