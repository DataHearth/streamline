...
func (_q *MediaFileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*MediaFile, error) {
	...
	if query := _q.withMovie; query != nil {
		if err := _q.loadMovie(ctx, query, nodes, nil,
			func(n *MediaFile, e *Movie) { n.Edges.Movie = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *MediaFileQuery) loadMovie(ctx context.Context, query *MovieQuery, nodes []*MediaFile, init func(*MediaFile), assign func(*MediaFile, *Movie)) error {
	ids := make([]uint32, 0, len(nodes))
	nodeids := make(map[uint32][]*MediaFile)
	for i := range nodes {
		if nodes[i].movie_media_files == nil {
			continue
		}
		fk := *nodes[i].movie_media_files
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "movie_media_files" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *MediaFileQuery) loadEpisode(ctx context.Context, query *EpisodeQuery, nodes []*MediaFile, init func(*MediaFile), assign func(*MediaFile, *Episode)) error {
	ids := make([]uint32, 0, len(nodes))
	nodeids := make(map[uint32][]*MediaFile)
	for i := range nodes {
		if nodes[i].episode_media_files == nil {
			continue
		}
		fk := *nodes[i].episode_media_files
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
...
func (_q *MediaFileQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(mediafile.Table, mediafile.Columns, sqlgraph.NewFieldSpec(mediafile.FieldID, field.TypeUint32))
	_spec.From = _q.sql
...
func (_q *MediaFileQuery) Modify(modifiers ...func(s *sql.Selector)) *MediaFileSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *MediaFileSelect) Modify(modifiers ...func(s *sql.Selector)) *MediaFileSelect {
