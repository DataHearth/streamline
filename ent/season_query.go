...
func (_q *SeasonQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Season, error) {
	...
	if query := _q.withTvShow; query != nil {
		if err := _q.loadTvShow(ctx, query, nodes, nil,
			func(n *Season, e *TVShow) { n.Edges.TvShow = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *SeasonQuery) loadTvShow(ctx context.Context, query *TVShowQuery, nodes []*Season, init func(*Season), assign func(*Season, *TVShow)) error {
	ids := make([]uint32, 0, len(nodes))
	nodeids := make(map[uint32][]*Season)
	for i := range nodes {
		if nodes[i].tv_show_seasons == nil {
			continue
		}
		fk := *nodes[i].tv_show_seasons
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tv_show_seasons" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SeasonQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(season.Table, season.Columns, sqlgraph.NewFieldSpec(season.FieldID, field.TypeUint32))
	_spec.From = _q.sql
...
func (_q *SeasonQuery) Modify(modifiers ...func(s *sql.Selector)) *SeasonSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *SeasonSelect) Modify(modifiers ...func(s *sql.Selector)) *SeasonSelect {
