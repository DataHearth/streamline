...
func (_q *InviteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invite, error) {
	...
	if _q.withCreatedBy != nil || _q.withUsedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withUsedBy; query != nil {
		if err := _q.loadUsedBy(ctx, query, nodes, nil,
			func(n *Invite, e *User) { n.Edges.UsedBy = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *InviteQuery) loadCreatedBy(ctx context.Context, query *UserQuery, nodes []*Invite, init func(*Invite), assign func(*Invite, *User)) error {
	ids := make([]uint32, 0, len(nodes))
	nodeids := make(map[uint32][]*Invite)
	for i := range nodes {
		if nodes[i].invite_created_by == nil {
			continue
		}
		fk := *nodes[i].invite_created_by
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "invite_created_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *InviteQuery) loadUsedBy(ctx context.Context, query *UserQuery, nodes []*Invite, init func(*Invite), assign func(*Invite, *User)) error {
	ids := make([]uint32, 0, len(nodes))
	nodeids := make(map[uint32][]*Invite)
	for i := range nodes {
		if nodes[i].invite_used_by == nil {
			continue
		}
		fk := *nodes[i].invite_used_by
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "invite_used_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *InviteQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(invite.Table, invite.Columns, sqlgraph.NewFieldSpec(invite.FieldID, field.TypeUint32))
	_spec.From = _q.sql
