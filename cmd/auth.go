...
// flagU32 narrows the operator's --limit flag into the store's uint32
// pagination domain, saturating instead of wrapping.
func flagU32(v uint) uint32 {
	if v >= math.MaxUint32 {
...
// storeCount narrows an in-memory count (a slice length) the same way; the
// early-return shape is what gosec's range analysis accepts.
func storeCount(v int) uint32 {
	if v <= 0 {
		return 0
	}
...
func authList(ctx context.Context, cmd *cli.Command) error {
	...
	return withAuthDeps(ctx, cmd, func(
		...
		for {
			...
			if limit > 0 && limit-storeCount(len(users)) < page {
				page = limit - storeCount(len(users))
			}
			...
			offset += storeCount(len(items))
			if len(items) == 0 || len(users) >= total {
				break
			}
			if limit > 0 && storeCount(len(users)) >= limit {
				break
			}
		}
